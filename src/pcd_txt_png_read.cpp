...
{
    ros::init(argc, argv, "pcd_txt_png_read_node");
    ros::NodeHandle nh ("~");
    ...
    FILE* file;
    file = fopen("/home/kimm/pcd_img_data/txt/odom1.txt","r");
    ...
    {
        pcl::io::loadPCDFile<pcl::PointXYZRGB> ("/home/kimm/pcd_img_data/0413_pcd/" + std::to_string(i) + ".pcd", cloud); //* load the file
        cv::Mat image = cv::imread("/home/kimm/pcd_img_data/0413_seg/" + std::to_string(i) + ".png",cv::IMREAD_COLOR);

        fscanf(file,"%s %f %f %f %f %f %f %f", &rostime, &pose_x, &pose_y, &pose_z, &orien_x, &orien_y, &orien_z, &orien_w);
        cout << rostime << " " << pose_x << " " << pose_y << " " << pose_z << " " << orien_x << " " << orien_y<< " " << orien_z << " " <<orien_w << "\n" << endl;
        ...
        for(int j = 0; j < cloud.size(); j ++)
        {
            ...
            // std::cout << cloud.points[j].x << " " << cloud.points[j].y << " " << cloud.points[j].z << std::endl;
